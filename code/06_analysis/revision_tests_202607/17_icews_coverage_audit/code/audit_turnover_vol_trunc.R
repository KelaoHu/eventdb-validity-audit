...
vol_of <- function(d) {
  d <- d[order(month)]
  data.table(
    vol_sd12m = mean(rollapply(d$value, 12, sd, fill = NA, align = "right"), na.rm = TRUE),
    vol_range12m = mean(rollapply(d$value, 12, function(x) max(x) - min(x), fill = NA, align = "right"), na.rm = TRUE))
}
