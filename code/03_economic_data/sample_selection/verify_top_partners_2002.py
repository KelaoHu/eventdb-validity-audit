...
OUTPUT_FILE = OUTPUT_DIR / "UN_Comtrade_China_Top30_Bilateral_Trade_2002_primary_cif_fob复现.csv"
...
def build_primary_ranking(session: requests.Session) -> pd.DataFrame:
    """
    以中国 reporter 口径构造 2002 年双边贸易总额前 30 名。
    主表以 primary 为准，同时保留 CIF/FOB 作为附表字段。
    """
    print("\n" + "=" * 72)
    print(f"第一步：抓取中国作为 reporter 的 {REPORT_YEAR} 年双边贸易数据")
    print("=" * 72)
    ...
    topn = merged.head(TOP_N).copy()

    print(f"中国 reporter 口径下，共识别到 {len(merged)} 个伙伴国家/地区。")
    print(f"前 {TOP_N} 名已筛出。")
    print(topn[["rank", "partner_code", "partner_name", "cn_export_primary", "cn_import_primary", "cn_trade_primary"]].to_string(index=False))
...
def fetch_mirror_verification(session: requests.Session, partner_codes: Sequence[int]) -> pd.DataFrame:
    ...
    """
    print("\n" + "=" * 72)
    print(f"第二步：抓取前 {TOP_N} 名伙伴的镜像数据进行核验")
    print("=" * 72)
...
def build_final_table() -> pd.DataFrame:
    """构建最终输出表。"""
    print("=" * 72)
    print(f"UN Comtrade {REPORT_YEAR} 年中国双边贸易前 {TOP_N} 名程序开始运行")
    print("=" * 72)
    ...
    topn = build_primary_ranking(session)
    mirror = fetch_mirror_verification(session, topn["partner_code"].tolist())

    print("\n" + "=" * 72)
    print("第三步：合并主表与镜像表")
    print("=" * 72)

    final_df = topn.merge(mirror, on="partner_code", how="left")
